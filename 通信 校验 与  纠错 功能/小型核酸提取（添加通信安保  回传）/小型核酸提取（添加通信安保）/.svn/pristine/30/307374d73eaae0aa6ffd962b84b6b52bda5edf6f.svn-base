  ...
  u16 set_back_this;	
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if(HeadOk)
		{
			...
			{
//				  Send_Str_uart4("abcdefg\r\n");
				set_back_this=HeadPosition+1;
				SendOneByte(';');
				SendOneByte_uart4(';');
				if(HeadPosition > WriteP)
				{
					while(set_back_this < USART_REC_LEN )
				   {
							SendOneByte(USART_RX_BUF[set_back_this]);
						  SendOneByte_uart4(USART_RX_BUF[set_back_this]);
							set_back_this++;
				   };
					 set_back_this=0;
					 while(set_back_this <=WriteP)
				   {
							SendOneByte(USART_RX_BUF[set_back_this]);
						  SendOneByte_uart4(USART_RX_BUF[set_back_this]);
							set_back_this++;

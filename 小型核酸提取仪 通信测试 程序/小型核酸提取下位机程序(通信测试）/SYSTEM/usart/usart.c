...
//				}
				...
				if((USART_RX_BUF[VarPosition] == 's')&&(USART_RX_BUF[ValuePosition] == 0x33)) //0x31 1 2 3
				{
					  for(i=0;i<strlen(USART_RX_BUF);i++)
					    if(USART_RX_BUF[i],usart_check[i])
							{
								Send_Str_uart4("!!!error\r\n");
								sprintf(s,"NO:%d\r\n",i);
							  Send_Str_uart4(s);
								Send_Str_uart4("!!!error\r\n");
							}
...
//							NVIC_SystemReset();
				...
				if((USART_RX_BUF[VarPosition] == 's')&&(USART_RX_BUF[ValuePosition] == 0x34)) //0x31 1 2 3
				{
					  u16 i;
					  delay_ms(1000);
...
//	    while(USART_GetFlagStatus(UART4,USART_FLAG_TC)==RESET); //等待上一次发送结束
...
//				u16 ts = 0; 
			//	int num = 0;
			  ...
			  crc_check[0]=':';
			  strncat(crc_check,FrameDeal,strlen(FrameDeal)-2);
			  crc_result=(crc_check,strlen(FrameDeal),0xffff);
